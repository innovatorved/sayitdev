...
7. **Architecture review.** Does the change fit the three delivery modes? Respect the non-negotiable principles? Honor the `SayItDevCore` (pure) / `SayItDevCLI` (CLI types) / `dev` (FoundationModels + Hummingbird) layering?

8. **Test coverage check.** Per CLAUDE.md section 7:
   - New flag → CLI argument tests (happy path + validation errors)
   - New public `SayItDevCore` API → unit test in `Tests/apfelTests/*Tests.swift`
