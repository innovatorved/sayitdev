...
**Triggers:**
1. GitHub webhook, `issues.labeled`, on `__UPSTREAM_DEV_REPO__`, filter: label name is `bug` (applied by the triage routine).
2. GitHub webhook, `issue_comment.created`, on `__UPSTREAM_DEV_REPO__`, filter: comment body contains `@Arthur-Ficial investigate`.
...
7. **Open the PR as a DRAFT.**
   ...
   gh pr create --draft --repo __UPSTREAM_DEV_REPO__ --base main --title "fix(<area>): <summary> (#<n>)" --body "<see template>"
   ```
...
5. Is my commit message honest about the routine authorship? - it should include "Drafted by the dev bug-solver routine. Needs review + local test run by @franzenzenhofer".
