...
func runResponsesModelsTests() {
    ...
    test("decodes a string input") {
        let r = try decodeResponses(#"{"model":"sayitdev-on-device","input":"hello"}"#)
        try assertEqual(r.model, "sayitdev-on-device")
    ...
    test("decodes a message-list input") {
        let r = try decodeResponses(#"{"model":"sayitdev-on-device","input":[{"role":"user","content":"hi"}]}"#)
    ...
    test("decodes typed message items with content parts") {
        ...
        {"model":"sayitdev-on-device","input":[
    ...
    test("decodes instructions, sampling params, and metadata") {
        let r = try decodeResponses("""
        {"model":"sayitdev-on-device","input":"x","instructions":"be terse",
    ...
    test("decodes text.format json_schema") {
        let r = try decodeResponses("""
        {"model":"sayitdev-on-device","input":"x",
    ...
    test("mapper: instructions become a leading system message") {
        let r = try decodeResponses(#"{"model":"sayitdev-on-device","input":"hi","instructions":"be terse"}"#)
    ...
    test("validator: unknown text.format type is a 400") {
        let r = try decodeResponses("""
        {"model":"sayitdev-on-device","input":"x","text":{"format":{"type":"yaml"}}}
        """)
    ...
    test("validator: json_schema without a schema is a 400") {
        let r = try decodeResponses("""
        {"model":"sayitdev-on-device","input":"x","text":{"format":{"type":"json_schema","name":"p"}}}
        """)
    ...
    test("validator: out-of-range sampling params are 400s") {
        for json in [
            #"{"model":"sayitdev-on-device","input":"x","temperature":3}"#,
            #"{"model":"sayitdev-on-device","input":"x","top_p":1.5}"#,
            #"{"model":"sayitdev-on-device","input":"x","max_output_tokens":0}"#,
        ] {
    ...
    test("validator: previous_response_id is a 501 (dev is stateless)") {
        let r = try decodeResponses(#"{"model":"sayitdev-on-device","input":"x","previous_response_id":"resp_1"}"#)
        let f = ResponsesRequestValidator.validate(r)
    ...
    test("validator: background is a 501") {
        let r = try decodeResponses(#"{"model":"sayitdev-on-device","input":"x","background":true}"#)
        try assertEqual(ResponsesRequestValidator.validate(r), .unsupported("background"))
    }

    test("validator: reasoning is a 501") {
        let r = try decodeResponses(#"{"model":"sayitdev-on-device","input":"x","reasoning":{"effort":"low"}}"#)
    ...
    test("validator: function tools with stream=true are a 501 (v1)") {
        let r = try decodeResponses("""
        {"model":"sayitdev-on-device","input":"x","stream":true,
    ...
    test("validator: a plain valid request passes") {
        let r = try decodeResponses("""
        {"model":"sayitdev-on-device","input":"hello","instructions":"be brief",
