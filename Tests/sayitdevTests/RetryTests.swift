...
func runRetryTests() {
    ...
    // ---- isRetryableError using SayItDevError.classify() ----

    test("isRetryableError: rateLimited SayItDevError is retryable") {
        try assertTrue(isRetryableError(SayItDevError.rateLimited))
    }

    test("isRetryableError: concurrentRequest SayItDevError is retryable") {
        try assertTrue(isRetryableError(SayItDevError.concurrentRequest))
    }

    test("isRetryableError: assetsUnavailable SayItDevError is retryable") {
    ...
    test("isRetryableError: contextOverflow is NOT retryable") {
        try assertTrue(!isRetryableError(SayItDevError.contextOverflow))
    }

    test("isRetryableError: unsupportedLanguage is NOT retryable") {
        try assertTrue(!isRetryableError(SayItDevError.unsupportedLanguage("Klingon")))
    }

    test("isRetryableError: toolExecution is NOT retryable") {
        try assertTrue(!isRetryableError(SayItDevError.toolExecution("tool failed")))
    }

    test("isRetryableError: unknown error is NOT retryable") {
        try assertTrue(!isRetryableError(SayItDevError.unknown("something happened")))
    }
    ...
    test("isRetryableError: assetsUnavailable GenerationError is retryable") {
        ...
        try assertEqual(SayItDevError.classify(err), .assetsUnavailable)
        try assertTrue(isRetryableError(err))
    }
    ...
    testAsync("withRetry: retries on retryable error then succeeds") {
        ...
        let result: String = try await withRetry(maxRetries: 3, delays: [0.01, 0.01, 0.01]) {
            attempts += 1
            if attempts < 3 {
                throw SayItDevError.rateLimited
            }
            return "recovered"
        }
    ...
    testAsync("withRetry: does NOT retry on non-retryable error") {
        ...
        do {
            let _: String = try await withRetry(maxRetries: 3, delays: [0.01, 0.01, 0.01]) {
                attempts += 1
                throw SayItDevError.guardrailViolation
            }
    ...
    testAsync("withRetry: stops after successful attempt") {
        ...
        let result: String = try await withRetry(maxRetries: 5, delays: [0.01, 0.01, 0.01, 0.01, 0.01]) {
            ...
            throw SayItDevError.concurrentRequest
        }
