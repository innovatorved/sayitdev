// ============================================================================
// SayItDevCorePublicAPIUsageTests.swift — Compile-time lockdown of the SayItDevCore
...
// `.library(name: "SayItDevCore", ...)`. Purpose:
//
...
// run sayitdev-tests` and against HEAD, so it catches breakage within a single
...
func runSayItDevCorePublicAPIUsageTests() {

    // MARK: - SayItDevError + isRetryableError(_:)

    test("SayItDevError cases, accessors, and classify(_:) are public") {
        ...
        // Equatable is public.
        try assertEqual(SayItDevError.rateLimited, SayItDevError.rateLimited)
        let _: Set<SayItDevError> = [.rateLimited, .contextOverflow]
        // classify(_:) is public static.
        let classified = SayItDevError.classify(MCPError.timedOut("x"))
    ...
    test("OpenAI types are constructible and Sendable where declared") {
        ...
        try assertEqual(
            try JSONDecoder().decode(
                ...
                from: Data(#"{"model":"sayitdev-on-device","messages":[{"role":"user","content":"hi"}]}"#.utf8)
            ),
            req
        )
    }
    ...
    test("ChatRequestValidator public surface compiles") {
        try assertEqual(ChatRequestValidator.validModel, "sayitdev-on-device")
    ...
    test("StreamOutcome and StreamErrorResolver public surface compiles") {
        ...
        if case .fatal(let e) = fatal { let _: SayItDevError = e }
    }
