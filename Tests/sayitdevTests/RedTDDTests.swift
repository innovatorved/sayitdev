...
func runRedTDDTests() {
    ...
    test("#177 DEV_PORT out-of-range falls back to default (like --port)") {
        // --port rejects values outside 1..65535; the env var must too.
        let args = try CLIArguments.parse([], env: ["DEV_PORT": "99999"])
        try assertEqual(args.serverPort, 11434,
            "out-of-range DEV_PORT must fall back to 11434, not bind an invalid port")
    }

    test("#177 DEV_TEMPERATURE negative is rejected (like --temperature)") {
        // --temperature requires >= 0; the env var currently accepts -5.
        let args = try CLIArguments.parse([], env: ["DEV_TEMPERATURE": "-5"])
        try assertNil(args.temperature,
            "negative DEV_TEMPERATURE must be rejected like the --temperature flag")
    }

    test("#177 DEV_CONTEXT_MAX_TURNS zero is rejected (like the flag)") {
        // --context-max-turns requires > 0; the env var currently accepts 0.
        let args = try CLIArguments.parse([], env: ["DEV_CONTEXT_MAX_TURNS": "0"])
        try assertNil(args.contextMaxTurns,
            "non-positive DEV_CONTEXT_MAX_TURNS must be rejected like the flag")
    }
