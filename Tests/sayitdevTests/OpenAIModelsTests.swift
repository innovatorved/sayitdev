...
func runOpenAIModelsTests() {
    ...
    test("OpenAIMessage decodes image_url parts and extracts imageURLs and promptText") {
        let json = """
        {
            "role": "user",
            "content": [
        ...
        """
        let message = try decode(OpenAIMessage.self, from: json)
        try assertTrue(message.containsImageContent)
        try assertEqual(message.promptText, "What is in this image?")
        try assertEqual(message.imageURLs, ["data:image/png;base64,iVBORw0KGgo="])
    }

    test("validator allows image content when allowImages is true") {
        let request = try decode(
            ChatCompletionRequest.self,
            from: #"{"model":"\#(ChatRequestValidator.validModel)","messages":[{"role":"user","content":[{"type":"text","text":"look"},{"type":"image_url","image_url":{"url":"photo.png"}}]}]}"#
        )
        try assertEqual(ChatRequestValidator.validate(request, allowImages: false), .imageContent)
        try assertNil(ChatRequestValidator.validate(request, allowImages: true))
    }
    ...
    test("ChatCompletionRequest decodes stream_options.include_usage=true") {
        let json = #"{"model":"sayitdev-on-device","messages":[{"role":"user","content":"hi"}],"stream":true,"stream_options":{"include_usage":true}}"#
    ...
    test("ChatCompletionRequest decodes stream_options.include_usage=false") {
        let json = #"{"model":"sayitdev-on-device","messages":[{"role":"user","content":"hi"}],"stream":true,"stream_options":{"include_usage":false}}"#
        let req = try decode(ChatCompletionRequest.self, from: json)
        try assertEqual(req.stream_options?.include_usage, false)
    }

    test("ChatCompletionRequest stream_options is nil when absent") {
        let json = #"{"model":"sayitdev-on-device","messages":[{"role":"user","content":"hi"}],"stream":true}"#
        let req = try decode(ChatCompletionRequest.self, from: json)
        try assertNil(req.stream_options)
    }

    test("ChatCompletionRequest stream_options.include_usage is nil when object empty") {
        let json = #"{"model":"sayitdev-on-device","messages":[{"role":"user","content":"hi"}],"stream":true,"stream_options":{}}"#
        let req = try decode(ChatCompletionRequest.self, from: json)
    ...
    test("ChatCompletionRequest decodes top_p") {
        let json = #"{"model":"sayitdev-on-device","messages":[{"role":"user","content":"hi"}],"top_p":0.9}"#
        let req = try decode(ChatCompletionRequest.self, from: json)
        try assertEqual(req.top_p, 0.9)
    }

    test("ChatCompletionRequest top_p defaults to nil when absent") {
        let json = #"{"model":"sayitdev-on-device","messages":[{"role":"user","content":"hi"}]}"#
        let req = try decode(ChatCompletionRequest.self, from: json)
...
func runChatRequestValidatorTests() {
    let M = ChatRequestValidator.validModel  // "sayitdev-on-device"
