...
func runCompletionsTests() {
    ...
    test("bash output has the correct registration markers") {
        ...
        try assertTrue(out.contains("_dev()"), "missing _dev() function")
        try assertTrue(out.contains("complete -F _dev dev"), "missing complete -F")
    }

    test("zsh output has the #compdef marker") {
        let out = ShellCompletions.generate(for: .zsh)
        try assertTrue(out.hasPrefix("#compdef dev"), "missing #compdef header")
        try assertTrue(out.contains("_arguments"), "missing _arguments")
    }

    test("fish output has the complete -c dev marker") {
        let out = ShellCompletions.generate(for: .fish)
        try assertTrue(out.contains("complete -c dev"), "missing complete -c dev")
    }
