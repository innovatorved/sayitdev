...
func runStreamErrorResolverTests() {
    ...
    test("contextOverflow + empty prev -> fatal(.contextOverflow) — prompt-side overflow") {
        ...
        case .fatal(let err): try assertEqual(err, SayItDevError.contextOverflow)
        }
    }
    ...
    test("refusal + empty prev -> fatal(.refusal)") {
        ...
        if case .fatal(let err) = outcome { try assertEqual(err, SayItDevError.refusal("blocked")) }
    }
    ...
    // MARK: - Every other SayItDevError case is fatal regardless of prev
