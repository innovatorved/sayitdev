...
func runModelAvailabilityTests() {
    ...
    test("remediation for .deviceNotEligible explains it is not an dev limitation") {
        let r = ModelAvailability.deviceNotEligible.remediation
        try assertTrue(r.contains("not an dev limitation"))
    }
    ...
    test("remediation for .unknownUnavailable tells user to file an issue") {
        ...
        try assertTrue(r.contains("github.com/innovatorved/sayitdev/issues"))
    }
