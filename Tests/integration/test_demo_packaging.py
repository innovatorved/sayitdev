"""
dev Integration Tests -- Demo Script Packaging

Pins the brew install shape that ships demo/*  as dev-<name> companion
commands. Verifies:
  ...
  2. The homebrew tap formula generator writes a `def install` block that
     installs demo/ and creates dev-<name> symlinks for every demo script.
...
def test_formula_generator_installs_every_demo_symlink(tmp_path):
    ...
    assert "bin.install_symlink" in text, (
        "formula should symlink demos into bin with dev- prefix"
    )
...
def test_formula_generator_exposes_demos_in_caveats(tmp_path):
    ...
    for name in EXPECTED_DEMOS:
        assert f"dev-{name}" in text, (
            f"caveats should advertise dev-{name} so users can discover it"
        )
...
def test_formula_brew_test_asserts_demo_symlink(tmp_path):
    ...
    assert "dev-cmd" in body, "brew test should assert dev-cmd is installed"
...
def test_release_tarball_bundles_demos():
    ...
    asset = ROOT / f"dev-{version}-arm64-macos.tar.gz"
    ...
    assert "dev" in entries, "tarball must contain the dev binary"
    assert "dev.1" in entries, "tarball must contain the man page"
    for name in EXPECTED_DEMOS:
