...
- Fixed light/dark mode theme switching with instant color-scheme update and dynamic toggle icon.
- Polished sidebar branding and streamlined Copy/Listen action buttons.
...
- macOS 27 Golden Gate SDK compatibility: adopted `samplingMode:` in `GenerationOptions`, handled `.reasoning(_)` enum case in `TokenCounter` and `Benchmark`, and added `@preconcurrency import AVFAudio` for Swift 6 concurrency clean build.
...
- `dev demos [dir]` writes the bundled demo scripts (cmd, explain, oneliner, wtd, naming, port, gitsum, mac-narrator) to a directory. The demos are embedded in the binary, so it behaves identically on homebrew-core, the tap, and source builds (#204).
...
- `DEV_DEBUG` env var enables debug logging (#164).
...
- Ship `demo/` scripts as `dev-<name>` companion commands in Homebrew (#155).
...
- TDD coverage for `SayItDevError.refusal` + extract `exitCode` mapping into SayItDevCLI (#124).

## [1.2.0] - 2026-04-24

### Added

- Preserve refusal explanation via `SayItDevError.refusal(String)` (#120).
...
- `SayItDevCore` exposed as a public Swift Package library product (#114, #105).
- Downstream-consumer smoke coverage for importing `SayItDevCore` from another package.
- DocC catalog, examples, and package metadata for `SayItDevCore`.
...
- Replace the unsafe global debug flag with `SayItDevDebugConfiguration`.
...
- `dev(1)` man page with drift-prevention (#103).
...
- `brew install dev` via homebrew-core.
