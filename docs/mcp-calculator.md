...
dev natively speaks the [https://modelcontextprotocol.io/](https://modelcontextprotocol.io/). Attach tool servers with `--mcp` and dev discovers tools, executes them, and returns the final answer.
...
dev --serve --mcp ./mcp/calculator/server.py

# Chat - tools persist across the conversation
dev --chat --mcp ./mcp/calculator/server.py

# Multiple MCP servers
dev --mcp ./calc.py --mcp ./weather.py "What is sqrt(2025)?"

# Slow or remote MCP server - increase timeout (default: 5s, max: 300s)
dev --mcp-timeout 30 --mcp ./remote-server.py "hello"
...
dev-run "What is 15 times 27?"    # same as dev + all enabled MCPs
dev-run --list                    # see what's on / off
```

This keeps dev itself flag-only - the registry layer lives in its own wrapper.
...
dev --mcp https://mcp.example.com/v1 "what tools do you have?"

# With bearer token auth - prefer the env var (flag is visible in ps aux)
DEV_MCP_TOKEN=mytoken dev --mcp https://mcp.example.com/v1 "..."
dev --mcp https://mcp.example.com/v1 --mcp-token mytoken "..."

# Mixed local + remote
dev --mcp /path/to/local.py --mcp https://remote.example.com/v1 "..."
```

> **Security:** Prefer `DEV_MCP_TOKEN` over `--mcp-token` because CLI flags are visible in `ps aux`. dev refuses to send a bearer token over plaintext `http://`; use `https://`.
...
$ dev --mcp ./mcp/calculator/server.py "Divide 1000 by 7"
...
$ dev --mcp ./mcp/calculator/server.py "Add 999 and 1"
...
```
dev --mcp ./calc.py "What is 15 times 27?"
  |
  v
...
When running `dev --serve --mcp ./calc.py`, the server auto-injects MCP tools for clients that don't send their own:
...
```bash
dev --mcp ./my-tool.py "question that needs the tool"
```
...
- **Default 5s timeout.** MCP startup and tool calls timeout after 5 seconds. Use `--mcp-timeout <seconds>` or `DEV_MCP_TIMEOUT` for slow/remote servers.
