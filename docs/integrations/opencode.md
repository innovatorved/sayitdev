...
```json
{
  ...
  "provider": {
    "dev": {
      ...
      "models": {
        "sayitdev-on-device": { "name": "sayitdev-on-device" }
      }
    }
  }
}
...
The model id `sayitdev-on-device` must match exactly what dev reports at `/v1/models`. `apiKey` is a placeholder: a local dev server started without `--serve-token` needs no auth, but opencode's OpenAI-compatible provider still wants the field present.
...
dev's on-device model has a **4096-token context window**. opencode is a full coding agent, and it **injects your instruction files into the system prompt on every request**. It loads them in this order (each category accumulates - they do not replace each other):

1. Local `AGENTS.md` / `CLAUDE.md` (walking up from the current directory)
2. Global `~/.config/opencode/AGENTS.md`
3. Claude Code fallback: **`~/.claude/CLAUDE.md`**

That third one is the trap. opencode has undocumented Claude Code compatibility: if you use Claude Code, your global `~/.claude/CLAUDE.md` gets pasted into opencode's system prompt verbatim. A big one (this machine's was ~12 KB / ~3,300 tokens) fills the 4096-token window before you type a word, and dev returns an honest HTTP 400:
...
Because of that window, dev is a great opencode backend for **short Q&A and small, focused edits** - not for large-repo, many-tool, long-running agent sessions. That is a property of the on-device model, not the wiring. The `dev --count-tokens` flag (see [docs/cli-reference.md](../cli-reference.md)) preflights how much a prompt will cost against the window.
...
5. **`apiKey` must be present** in the provider `options` even though a local dev server needs no auth - opencode's `@ai-sdk/openai-compatible` provider expects the field. Any placeholder works.
6. **Model id must match `/v1/models` exactly** (`sayitdev-on-device`). A mismatch fails the request.
...
dev's request log for that turn - every call `200 OK`, well inside the window, `$0.00`:
...
Without `OPENCODE_DISABLE_CLAUDE_CODE_PROMPT=1`, the same turn sent 13,461 bytes and dev returned `400 - Input exceeds the model's context window`.
