...
Call Apple's on-device Foundation Model from Python using the official `openai` SDK, pointed at a local `dev --serve`. 100% on-device, zero API cost, no network required for inference.
...
- `brew install dev`
- `dev --serve` running (default port `11434`)
- Python 3.11+
- `pip install openai` (or `uv add openai`)

## 1. One-shot chat completion

Point the `openai` SDK at your local dev server and call `chat.completions.create`:
...
Request `response_format: {"type": "json_object"}` and parse. dev may wrap output in markdown fences - the fence-strip regex below handles both cases cleanly:
...
dev returns honest HTTP errors for unsupported features. Embeddings return `501`:
...
- **`Connection refused` on port 11434** - run `dev --serve` first.
- **`Embeddings not supported`** - dev is text-only; use sentence-transformers or another embedder for vectors.
- **`JSONDecodeError` in JSON mode** - keep the fence-strip regex; dev sometimes wraps JSON in `` ```json ... ``` ``.
...
- dev v1.0.3 (original capture; the CLI and HTTP surfaces used here are release-gated by dev's test suite on every version)
