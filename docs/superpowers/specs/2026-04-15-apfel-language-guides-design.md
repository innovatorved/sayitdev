...
Ship a set of SEO-optimized, empirically tested "how to use the Apple Foundation Model from `<language>`" guides in the dev repo, backed by a separate lab repo that holds the runnable scripts and pytest harness that prove every code block actually works against a live `dev --serve`.
...
```
dev-guides-lab/
├── README.md                   # "This is the lab; dev docs are in the main repo"
├── Makefile                    # make test, make capture, make test-<lang>
├── conftest.py                 # pytest: boots `dev --serve` on :11434, waits /health
...
2. **Intro** - what dev is, why 100% on-device matters, what this guide covers (~3 sentences)
3. **Prerequisites** - macOS 26+, Apple Silicon, Apple Intelligence enabled, `brew install dev`, `dev --serve` running
...
11. **Tested with** - `dev <version>`, macOS <version>, `<language> <version>`, date. Link to the exact script in the lab repo at a pinned commit SHA

## Lab repo harness

- **pytest** with `conftest.py` session-scoped fixture that:
  1. Verifies `dev` is on PATH (skip with clear message if not)
  2. Boots `dev --serve --port 11434` as a background subprocess
...
- **`dev-guides-lab` GitHub Actions**: runs on push. Most tests skip on CI (GitHub runners lack Apple Intelligence) - uses the same honest-skip pattern as dev integration tests. Real qualification is `make test` locally on the Mac before publishing any guide update.
- **`dev` repo**: no CI change. Guides are pure Markdown.

## Workflow per language (for me, Arthur, to follow during build-out)

1. Write script in `dev-guides-lab/scripts/<lang>/`
2. `make test-<lang>` against a running `dev --serve` - must pass, empirically
3. `make capture` - real stdout lands in `outputs/<lang>/`
4. Write `docs/guides/<lang>.md` in dev repo, paste the real output
...
- All guides link to `docs/openai-api-compatibility.md` and `docs/install.md` on dev repo

## Versioning and maintenance

- Each guide's "Tested with" footer names exact dev + language runtime + macOS versions + date
- When dev releases a version with API-affecting changes, re-run `make test && make capture` in the lab, update the guides' footers and any output that changed
...
Do not work around dev bugs in the guide scripts. If a language exposes a real dev bug, file the ticket, mark the guide section as "Blocked on dev#<N>" in the lab repo, and move to the next section. The guide gets published only after the ticket is resolved in dev and re-verified.
...
- `make test` in the lab repo passes on Arthur's Mac, 0 skipped (when `dev --serve` is up)
- Every code block in every guide matches the real captured output byte-for-byte
- Lab repo README clearly explains its purpose and relationship to dev
- README.md on dev repo has a discoverable link to `docs/guides/index.md`
