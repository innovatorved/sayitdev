...
func handleChatCompletion(_ request: Request, context: some RequestContext) async throws -> (response: Response, trace: ChatRequestTrace) {
    ...
    if let failure = ChatRequestValidator.validate(chatRequest, allowImages: allowImages) {
        return chatFailure(
    ...
    if toolsAreMCPInjected {
        let userPrompt = chatRequest.messages.last(where: { $0.role == "user" })?.promptText ?? finalPrompt
...
private func nonStreamingResponse(
    ...
    do {
        ...
        outcome = try await withRetry(maxRetries: nsRetryMax) {
            try await collectStream(session, prompt: prompt, imageURLs: imageURLs, options: genOpts)
        }
    } catch {
        let classified = SayItDevError.classify(error)
        if case .refusal(let explanation) = classified {
...
private func streamingResponse(
    ...
    let responseStream = AsyncStream<ByteBuffer> { continuation in
        let streamTask = Task {
            ...
            if #available(macOS 27.0, *), !imageURLs.isEmpty {
                stream = session.streamResponse(options: genOpts) {
                ...
                stream = session.streamResponse(to: prompt, options: genOpts)
            }
            var prev = ""
            var chunkCount = 0
...
private func structuredNonStreamingResponse(
    ...
    do {
        content = try await withRetry(maxRetries: nsRetryMax) {
            if #available(macOS 27.0, *), !imageURLs.isEmpty {
                let result = try await session.respond(schema: schema, options: genOpts) {
                    for url in imageURLs {
                        Attachment(imageURL: url)
                    }
                    prompt
...
private func structuredStreamingResponse(
    ...
    let responseStream = AsyncStream<ByteBuffer> { continuation in
        let streamTask = Task {
            ...
            if #available(macOS 27.0, *), !imageURLs.isEmpty {
                stream = session.streamResponse(schema: schema, options: genOpts) {
                    for url in imageURLs {
                        Attachment(imageURL: url)
                    }
                    prompt
                ...
                stream = session.streamResponse(to: prompt, schema: schema, options: genOpts)
            }
            var prev = ""
