...
static inline FILE *dev_get_rl_outstream(void) {
    return rl_outstream;
}

static inline void dev_set_rl_outstream(FILE *stream) {
    rl_outstream = stream;
}

static inline FILE *dev_get_rl_instream(void) {
    return rl_instream;
}

static inline void dev_set_rl_instream(FILE *stream) {
    rl_instream = stream;
}

static volatile sig_atomic_t dev_sigint_reset_stdout = 0;
...
static struct termios dev_saved_termios __attribute__((used)) = {0};
static volatile sig_atomic_t dev_have_saved_termios __attribute__((used)) = 0;
...
static inline void dev_capture_termios(void) {
    if (tcgetattr(STDIN_FILENO, &dev_saved_termios) == 0) {
        dev_have_saved_termios = 1;
    }
}

static inline void dev_block_sigint(void) {
    sigset_t blocked;
    sigemptyset(&blocked);
...
static void dev_sigint_exit_handler(int sig) {
    (void)sig;
    ...
    if (dev_have_saved_termios) {
        (void)tcsetattr(STDIN_FILENO, TCSAFLUSH, &dev_saved_termios);
    }

    if (dev_sigint_reset_stdout) {
...
static inline void dev_install_sigint_exit_handler(int resetStdout) {
    struct sigaction sa;
    sigset_t unblock;
    sigemptyset(&sa.sa_mask);
    sa.sa_handler = dev_sigint_exit_handler;
    sa.sa_flags = 0;

    dev_sigint_reset_stdout = resetStdout ? 1 : 0;
...
static inline char *dev_readline_interruptible(const char *prompt) {
    struct sigaction sa;
    ...
    dev_sigint_reset_stdout = isatty(STDOUT_FILENO) != 0;
