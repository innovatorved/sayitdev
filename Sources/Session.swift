...
func processPrompt(
    ...
    debugLog("prompt", "stream=\(stream) retry=\(retryMax) mcp=\(hasMCPTools) images=\(imageURLs.count)")
    ...
    let outcome = try await withRetry(maxRetries: retryMax) {
        try await collectStream(session, prompt: prompt, imageURLs: imageURLs, sink: printSink, options: genOpts)
    }
