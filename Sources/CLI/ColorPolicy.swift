...
//
// Kept in SayItDevCLI (FoundationModels-free) so the gating rules are
...
/// Pure decisions for whether dev should emit ANSI color codes.
public enum ColorPolicy {
    /// True when `NO_COLOR` is present AND non-empty.
    ///
    /// Per https://no-color.org and dev's man page, only a non-empty value
