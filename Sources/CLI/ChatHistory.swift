...
// Part of SayItDevCLI - decides *whether* and *where* chat history persists.
//
...
/// unless the user explicitly opts in by setting `DEV_HISTFILE` to a path.
/// This is the honest, secure default - dev never writes a transcript of
...
public enum ChatHistory {
    ...
    public static let envVar = "DEV_HISTFILE"
    ...
    ///
    /// Returns `nil` (in-memory-only, the default) unless `DEV_HISTFILE` is
    ...
    /// `DEV_*` vars. A leading `~` is expanded to the user's home directory
    /// so `DEV_HISTFILE=~/.dev_history` works without shell expansion.
