...
// value, and what values). `dev completions <shell>` prints the generated
...
public enum ShellCompletions {
    ...
    /// Context-strategy values, sourced from SayItDevCore's enum so they cannot
    ...
    public static var flags: [CompletionFlag] {
        [
            ...
            CompletionFlag(["-i", "--image"], .file, "Attach an image to prompt for vision"),
            ...
            CompletionFlag(["--rate"], .generic, "TTS speaking rate"),
            CompletionFlag(["--audio-format"], .values(["wav", "pcm", "aac"]), "TTS output format"),
            CompletionFlag(["--timestamps"], .none, "Include timing in transcriptions"),
        ]
    }
    ...
    private static func bash() -> String {
        ...
        var lines: [String] = []
        lines.append("# dev(1) bash completion - generated by `dev completions bash`")
        lines.append("_dev() {")
        lines.append("    local cur prev")
        lines.append("    cur=\"${COMP_WORDS[COMP_CWORD]}\"")
        lines.append("    prev=\"${COMP_WORDS[COMP_CWORD-1]}\"")
        lines.append("")
        lines.append("    # `dev completions <shell>` subcommand")
        ...
        lines.append("}")
        lines.append("complete -F _dev dev")
    ...
    private static func zsh() -> String {
        var lines: [String] = []
        lines.append("#compdef dev")
        lines.append("# dev(1) zsh completion - generated by `dev completions zsh`")
        lines.append("_dev() {")
    ...
    private static func fish() -> String {
        var lines: [String] = []
        lines.append("# dev(1) fish completion - generated by `dev completions fish`")
        // Disable default file completion; re-enable per file/dir flag.
        lines.append("complete -c dev -f")
        // completions subcommand.
        lines.append("complete -c dev -n '__fish_use_subcommand' -a completions -d 'Generate shell completions'")
        lines.append("complete -c dev -n '__fish_seen_subcommand_from completions' -a '\(shellArgs.joined(separator: " "))'")
        for f in flags {
            var parts = ["complete -c dev"]
