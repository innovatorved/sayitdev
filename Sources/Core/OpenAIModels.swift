...
public struct OpenAIMessage: Codable, Sendable, Equatable, Hashable {
    ...
    /// Extracted text content from parts, ignoring image attachments.
    ...
    /// Extracted image URLs from image_url parts.
    public var imageURLs: [String] {
        guard case .parts(let parts) = content else { return [] }
        return parts.compactMap { part in
            guard part.type == "image_url" else { return nil }
...
/// An image URL payload for multimodal OpenAI messages.
...
public struct ContentPart: Codable, Sendable, Equatable, Hashable {
    ...
    /// The image URL payload for image_url parts.
