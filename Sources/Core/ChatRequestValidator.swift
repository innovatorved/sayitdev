...
public enum ChatRequestValidator {
    ...
    ///   - allowImages: Whether multimodal image content is accepted by the underlying model.
    /// - Returns: The first validation failure encountered, or `nil`.
    public static func validate(_ request: ChatCompletionRequest, allowImages: Bool = false) -> ChatRequestValidationFailure? {
        ...
        if !allowImages && request.messages.contains(where: \.containsImageContent) {
            return .imageContent
        }

        // A non-tool final message (last role is guaranteed user/tool here) must
        // carry non-empty text (or an image when images are allowed).
        if let last = request.messages.last, last.role != "tool" {
            let text = last.promptText ?? last.textContent
            let hasImage = allowImages && last.containsImageContent
            if !hasImage && (text == nil || text?.isEmpty == true) {
