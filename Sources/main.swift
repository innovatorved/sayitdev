...
do {
    ...
    case .stream:
        if let messagesJSON {
            ...
            try await singlePrompt(prompt, systemPrompt: parsed.systemPrompt, imagePaths: parsed.imagePaths, stream: true, options: sessionOpts, mcpManager: mcpManager)
        }
    ...
    case .single:
        ...
        if let messagesJSON {
            ...
            let status = try await singlePrompt(prompt, systemPrompt: effectiveSystemPrompt, imagePaths: parsed.imagePaths, stream: noArgsPipe, options: sessionOpts, mcpManager: mcpManager, codeOnly: parsed.codeOnly)
            if status != 0 {
                await shutdownMCP()
                exit(status)
            }
