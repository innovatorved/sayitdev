...
enum ContextManager {
    ...
    static func makeSession(
        ...
        if conversation.last?.role == "tool" {
            ...
            let candidateText = conversation.last?.promptText ?? conversation.last?.textContent
            let fallbackText = (conversation.last?.imageURLs.isEmpty == false) ? "Describe this image." : nil
            guard let text = candidateText ?? fallbackText, !text.isEmpty else {
    ...
    private static func historyEntry(
        ...
        switch message.role {
        case "user":
            guard let text = message.promptText ?? message.textContent else { return nil }
