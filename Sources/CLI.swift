...
func singlePrompt(
    ...
    debugLog("single", "prompt_length=\(prompt.count) stream=\(stream) mcp=\(hasMCPTools) code=\(codeOnly) images=\(imagePaths.count)")

    var imageURLs: [URL] = []
    for path in imagePaths {
        if let resolved = try? await ImagePayloadResolver.resolve(urlString: path) {
            imageURLs.append(resolved)
        } else {
            printStderr("\(styledErr("dev:", .red)) failed to resolve image: \(path)")
        }
    }
    ...
    let result = try await processPrompt(
        prompt: finalPrompt, systemPrompt: systemPrompt, imageURLs: imageURLs, session: session,
...
func structuredSinglePrompt(
    ...
    let genOpts = makeGenerationOptions(options)
    debugLog("schema", "prompt_length=\(prompt.count) schema_name=\(schemaName) images=\(imagePaths.count)")
    ...
    let content = try await withRetry(maxRetries: retryMax) {
        if #available(macOS 27.0, *), !targetImageURLs.isEmpty {
            return try await session.respond(schema: schema, options: genOpts) {
            ...
            return try await session.respond(to: prompt, schema: schema, options: genOpts).content.jsonString
        }
    }
