...
dev runs 100% on-device. There are no cloud calls for inference. The security surface is:

- **HTTP server** (`dev --serve`) - OpenAI-compatible API on localhost
...
- Bearer token authentication available via `--token` or `DEV_TOKEN`
